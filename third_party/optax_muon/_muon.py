...
def _base_newton_schulz_iteration(x: jax.Array, coeffs: jax.Array) -> jax.Array:
  ...
  # Megatron's non-Flash PyTorch path executes each expression as an eager
  # BF16 tensor operation. Matmuls accumulate in FP32, but every tensor result
  # is stored in BF16 before it is consumed by the next operation.
  a_fp32 = jnp.matmul(x, x.T.conj(), preferred_element_type=jnp.float32)
  a = a_fp32.astype(jnp.bfloat16)

  scaled_a_quintic = (coeffs[2] * a.astype(jnp.float32)).astype(jnp.bfloat16)
  quintic_term = jnp.matmul(
      scaled_a_quintic,
      a,
      preferred_element_type=jnp.float32,
  ).astype(jnp.bfloat16)
  cubic_term = (coeffs[1] * a.astype(jnp.float32)).astype(jnp.bfloat16)
  b = (cubic_term + quintic_term).astype(jnp.bfloat16)

  bx = jnp.matmul(b, x, preferred_element_type=jnp.float32).astype(jnp.bfloat16)
  linear_term = (coeffs[0] * x.astype(jnp.float32)).astype(jnp.bfloat16)
  return (linear_term + bx).astype(jnp.bfloat16)
...
def orthogonalize_via_newton_schulz(
  ...
  def _orthogonalize(x):
    ...
    # Megatron keeps the polynomial constants in FP32 even when the NS matrix
    # is BF16. The BF16 iterator explicitly rounds each tensor-operation result
    # instead of quantizing the coefficients before the iteration starts.
    ns_coeffs_ = ns_coeffs.astype(jnp.float32)
...
def scale_by_muon(
  ...
  def init_fn(params):
    ...
    # Coefficient precision is independent of ns_dtype. In Megatron's
    # non-Flash path these values are FP32 scalar constants while the NS matrix
    # and each eager tensor-operation result can still be BF16.
    ns_coeffs_ = jnp.asarray(ns_coeffs, dtype=jnp.float32)
